...
# angle2pnw-faithful2: shadow_curvature packing scale, matches ford.h's FORD_BP_SHADOW_CURVATURE_TO_CAN
# decode. Verbatim from Alan Polk's bp-7.0 fordcan_ext.py (_BP_LKA_SHADOW_CURVATURE_SCALE).
...
def create_lka_msg(packer, CAN: CanBus, lat_active: bool, hud_control,
  ...
  """
  Creates a CAN message for the Ford LKA Command (Lane_Assist_Data1), ported faithfully from Alan
  Polk's BluePilot bp-7.0 opendbc_repo/opendbc/sunnypilot/car/ford/fordcan_ext.py::create_lka_msg.
  ...
  no separate CAN ID, no RX round-trip needed (panda does not self-receive its own TX).
