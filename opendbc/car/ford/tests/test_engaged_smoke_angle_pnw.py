"""
angle2pnw-faithful2 — engaged-path smoke test for the faithfully-ported bp-7.0 angle-primary
lateral strategy (LateralAngleExt), mirroring test_engaged_smoke_pnw.py's discipline for the
4-signal path (born from the 2026-07-11 numpy/capnp field failure that test closes).

The master gate (PnwVehicle.angle_lat) reads the driver-facing FordAngleLateral settings toggle
(default OFF) on the F-150 Lightning only. This file proves two separate things that must BOTH
hold:

  1. The gate is genuinely off by default — a normal CarInterface never constructs the angle-mode
     extension unless FordAngleLateral is set (test_angle_mode_off_by_default). Silently skipping
     this and only testing #2 would miss a bug where the capability accidentally activates.
  2. The MECHANISM itself is correct when exercised directly (bypassing the gate by assigning
     ...
     fault-injection fallback proof (inject a raising stub, assert the stock path resumes).
...
def test_angle_mode_off_by_default():
  """The master gate (PnwVehicle.angle_lat) reads the FordAngleLateral param, default OFF — a
  normal CarInterface must NEVER construct LateralAngleExt without the driver opting in. Guards
  against the capability accidentally activating (e.g. a future edit to pnw_vehicle.py or a params
  default flip)."""
  ci = _make_interface(FORD.FORD_F_150_LIGHTNING_MK1)
  ci.update([])
  assert ci.CC._latext_angle is None, \
    "angle mode's master gate is not off by default — LateralAngleExt got constructed"
...
def test_angle_mode_takes_precedence_when_toggled_on():
  """Regression test for a real bug caught during this port's own verification pass: the F-150
  Lightning's PnwVehicle.four_signal_lat is an always-True capability (not a toggle), so
  constructing the 4-signal LateralCurvExt BEFORE checking angle_lat would permanently starve
  angle mode of ever running -- flipping FordAngleLateral would silently do nothing. carcontroller
  must check angle_lat FIRST and skip constructing LateralCurvExt whenever angle mode is on."""
  import opendbc.car.ford.carcontroller as cc_mod
  from opendbc.car.pnw_vehicle import PnwVehicle

  class ForceAngleOn(PnwVehicle):
    def __init__(self, CP):
      super().__init__(CP)
      self.angle_lat = True

  orig = cc_mod.PnwVehicle
  cc_mod.PnwVehicle = ForceAngleOn
  try:
    ci = _make_interface(FORD.FORD_F_150_LIGHTNING_MK1)
    ci.update([])
    assert ci.CC._latext is None, \
      "4-signal LateralCurvExt was constructed even though the angle toggle is on — angle mode is starved"
    assert ci.CC._latext_angle is not None, \
      "angle toggle is on but LateralAngleExt was never constructed"
  finally:
    cc_mod.PnwVehicle = orig


@pytest.mark.skipif(not HAVE_CEREAL, reason="cereal not on PYTHONPATH — run from the pnw-pilot venv")
def test_angle_mode_engaged_smoke():
  """Bypass the (off-by-default) capability gate by assigning _latext_angle directly — proves the
...
def test_angle_mode_fault_injection_falls_back():
  """A failure in LateralAngleExt must fall back to the stock curvature-only path for the drive,
  never raise out of apply() / kill card — same never-kill-card discipline as
