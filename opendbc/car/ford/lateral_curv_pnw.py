"""
fordsafety2pnw — BluePilot Ford 4-signal lateral control, ported from BluePilot (alan-polk),
bluepilotdev/bp-dev opendbc_repo/opendbc/sunnypilot/car/ford/lateral_curv_ext.py.
...
Used by composition from CarController (BP mixes it in via multiple inheritance).
...
pnw adaptations (control math untouched):
  - module-level cereal/ModelConstants imports stay unguarded; the CALLER (carcontroller) wraps
    the import of this module — on a bare opendbc checkout the import fails and the carcontroller
    falls back to the stock curvature-only path.
  - PIDController is vendored below (BP imports it from its monorepo common/pid.py; opendbc
    cannot import openpilot.*). Numerically identical.
  - update_lateral_params() reads BP's UI Params keys, which do not exist in the pnw params
    registry — the pnw carcontroller does NOT call it, so the BP __init__ defaults rule
    (human-turn detection ON, lane positioning OFF, custom profile OFF, LC_PID_gain_UI 0).
    With those defaults path_angle and path_offset are sent as 0 — matching BP out-of-the-box.
"""
...
from opendbc.car.ford.values_pnw import BP_ANGLE_LIMITS, CURVATURE_MAX
...
class PIDController:
  """Vendored from BluePilot common/pid.py (identical to openpilot's) — opendbc cannot import
  openpilot.*. Attribution: comma.ai / BluePilot (alan-polk)."""
...
class LateralCurvExt:
  ...
  pnw: used by COMPOSITION from CarController (BP mixes it in via inheritance). CarController
  calls update() during the lateral control loop (20Hz) to get the full 4-signal steering
  command instead of the upstream curvature-only approach.
  """
  ...
  def __init__(self, CP):
    ...
    # Toggles (BP: updated from Params each frame; pnw: fixed at BP defaults, see module docstring)
  ...
  def update_lateral_params(self, params):
    """Read lateral-related Params from the UI (BP). NOT called by the pnw carcontroller — these
    keys are not in the pnw params registry; BP __init__ defaults rule. Kept for fidelity; each
    read is guarded so a partially-registered key set cannot crash the control loop."""
    for attr, key, cast in (
      ("enable_human_turn_detection", "enable_human_turn_detection", bool),
      ("lane_change_factor_high", "lane_change_factor_high", float),
      ("pc_blend_ratio_high_C_UI", "pc_blend_ratio_high_C_UI", float),
      ("pc_blend_ratio_low_C_UI", "pc_blend_ratio_low_C_UI", float),
      ("enable_lane_positioning", "enable_lane_positioning", bool),
      ("custom_path_offset", "custom_path_offset", float),
      ("enable_lanefull_mode", "enable_lane_full_mode", bool),
      ("custom_profile", "custom_profile", int),
      ("LC_PID_gain_UI", "LC_PID_gain_UI", float),
    ):
      try:
        if cast is bool:
          setattr(self, attr, params.get_bool(key))
        else:
          setattr(self, attr, cast(params.get(key, return_default=True)))
      except Exception:
        pass  # key not registered in this tree — keep the BP default
